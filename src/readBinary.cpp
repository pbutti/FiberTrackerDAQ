...
int main(){
    std::ifstream infile;
    infile.open("data/XBPF_test_run_1_41.bin", std::ios::binary | std::ios::in);
    ...
    for(const auto& k: result){ 
        ...
        if( k[0] == 1 ){
            ...
            */
            for(int i = 0; i < 500; i++){
                std::cout << eventsData[i*10+0] << " " << eventsData[i*10+1]  << " "<< std::bitset<32>(eventsData[i*10+4]) << std::bitset<32>(eventsData[i*10+5]) << 
                 std::bitset<32>(eventsData[i*10+6]) << std::bitset<32>(eventsData[i*10+7]) << std::bitset<32>(eventsData[i*10+8]) << std::bitset<32>(eventsData[i*10+9]) << std::endl;
            }
